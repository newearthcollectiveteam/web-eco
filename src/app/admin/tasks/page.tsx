          ...
          <div>
            ...
            >
              ...
              <textarea
                ...
                rows={5}
                className="block w-full bg-transparent px-3 py-2.5 text-sm text-white placeholder-neutral-500 focus:outline-none"
                placeholder="Detailed notes... Use the toolbar for formatting."
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="block w-full rounded-md border bg-white/5 px-3 py-2.5 text-white focus:border-[#FACF39] focus:ring-1 focus:ring-[#FACF39]/30 focus:outline-none"
                style={{ borderColor: "rgba(250, 207, 57, 0.2)" }}
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label
                ...
                className="mb-1.5 block text-sm font-medium text-neutral-300"
              >
                Start Date <span className="text-neutral-500">(optional)</span>
              </label>
              <input
                id="task-start-date"
                type="date"
                ...
                className="block w-full rounded-md border bg-white/5 px-3 py-2.5 text-white [color-scheme:dark] focus:border-[#FACF39] focus:ring-1 focus:ring-[#FACF39]/30 focus:outline-none"
                style={{ borderColor: "rgba(250, 207, 57, 0.2)" }}
              />
            </div>
            <div>
...
function TaskCard({
  ...
  const isOverdue =
    task.dueDate &&
    task.status !== statuses[statuses.length - 1]?.slug &&
  ...
  return (
    <div
      className={`rounded-xl border transition-colors ${isLastStatus ? "opacity-60" : ""}`}
      style={{ borderColor: "rgba(250, 207, 57, 0.1)" }}
    >
      <div className="flex items-start gap-3 px-4 py-3">
        <button
          onClick={nextStatus}
          className={`mt-0.5 shrink-0 rounded-full border p-0.5 transition-colors hover:bg-white/5 ${statusColor}`}
          aria-label={`Status: ${statusConfig?.name ?? task.status} (click to advance)`}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            >
              <span
                className={`text-sm font-medium ${isLastStatus ? "text-neutral-500 line-through" : "text-white"}`}
              >
                {task.title}
              </span>
            </button>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-neutral-500">
            ...
            {task.startDate && task.dueDate && (
              <span
                className={`inline-flex items-center gap-1 ${isOverdue ? "text-red-400" : ""}`}
              >
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <button
            ...
            className="min-h-[44px] min-w-[44px] rounded p-2 text-neutral-500 hover:bg-white/5 hover:text-white sm:min-h-0 sm:min-w-0 sm:p-1.5"
          >
          ...
          {confirmDelete ? (
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="rounded px-2 py-1 text-[10px] text-neutral-400 hover:text-white"
              >
                No
              </button>
            </div>
            ...
            <button
              ...
              className="min-h-[44px] min-w-[44px] rounded p-2 text-neutral-500 hover:bg-white/5 hover:text-red-400 sm:min-h-0 sm:min-w-0 sm:p-1.5"
            >
      ...
      {expanded && task.description && (
        <div className="border-t border-white/5 px-4 py-3 pl-11">
          <RichNotes text={task.description} />
        </div>
      )}
    </div>
  );
...
function KanbanCard({
  ...
  const isLastStatus = task.status === statuses[statuses.length - 1]?.slug;
  const isOverdue =
    task.dueDate && !isLastStatus && new Date(task.dueDate) < new Date();
  const formatDate = (d: Date | string) =>
    new Date(d).toLocaleDateString("en-US", { month: "short", day: "numeric" });
  ...
  return (
    <div
      ...
      className={`group rounded-lg border bg-[#0a0a0a] p-3 transition-shadow ${
        isDragging
          ? "border-[#FACF39]/40 shadow-lg shadow-[#FACF39]/10"
    ...
    >
      <div className="flex items-start gap-2">
        <button
          ...
          className="mt-0.5 shrink-0 cursor-grab text-neutral-600 hover:text-neutral-400 active:cursor-grabbing"
        >
          <GripVertical className="h-4 w-4" />
        </button>
        <button onClick={onEdit} className="min-w-0 flex-1 text-left">
          <p
            className={`text-sm leading-tight font-medium ${isLastStatus ? "text-neutral-500 line-through" : "text-white"} truncate`}
          >
            {task.title}
          </p>
        </button>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 pl-6">
        ...
        {task.dueDate && (
          <span
            className={`inline-flex items-center gap-1 text-[10px] ${isOverdue ? "text-red-400" : "text-neutral-500"}`}
          >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-4 py-3">
        ...
        <button
          onClick={onAddTask}
          className="rounded p-1 text-neutral-500 hover:bg-white/5 hover:text-[#FACF39]"
...
export default function TasksPage() {
          ...
          tasks: old.tasks.map((t) =>
            t.id === variables.id
              ? {
                  ...
                  completedAt:
                    variables.status === statuses[statuses.length - 1]?.slug
                      ? new Date()
                      : null,
                }
              : t
  ...
  const handleReorderStatuses = (reordered: StatusConfig[]) => {
    ...
    utils.tasks.listStatuses.setData(
      undefined,
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-400">
            {total} task{total !== 1 ? "s" : ""} across your team
          </p>
        </div>
        <button
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 p-1">
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${activeView === "list" ? "bg-white/10 text-white" : "text-gray-400 hover:text-white"}`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${activeView === "kanban" ? "bg-white/10 text-white" : "text-gray-400 hover:text-white"}`}
            >
      ...
      {showReorder && statuses.length > 0 && (
        <div className="rounded-lg border border-white/10 bg-white/[0.02] p-3">
          <p className="mb-2 text-xs text-neutral-500">
            Drag to reorder categories. Click names to rename.
          </p>
          <StatusReorderBar
            statuses={statuses}
          ...
          {tasksQuery.isLoading || statusesQuery.isLoading ? (
            <div className="flex gap-4">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-xl border border-white/10 p-4"
                >
                  <div className="mb-3 h-5 w-24 animate-pulse rounded bg-white/10" />
                  {Array.from({ length: 2 }).map((_, j) => (
                    <div
                      key={j}
                      className="mb-2 h-16 animate-pulse rounded-lg bg-white/5"
                    />
                  ))}
                </div>
              ))}

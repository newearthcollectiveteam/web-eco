                  ...
                  {!isExpanded && section.recent.length > 0 && (
                    <div className="border-t border-neutral-200 px-6 py-4 dark:border-neutral-800">
                      ...
                      <div className="space-y-2">
                        {section.recent.slice(0, 3).map((item: any) => (
                          <div
                            key={item.id}
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <span className="font-medium text-black dark:text-white">
                                {item.name || item.signerName}
                              </span>
                              <span className="text-sm text-neutral-500">
                                {item.email || item.signerEmail}
                              </span>
                            </div>
                            <span className="text-xs text-neutral-400">
                              {formatDate(item.createdAt || item.signedAt)}
                            </span>
                          </div>
                        ))}
                      </div>
                                ...
                                >
                                  ...
                                  <td className="px-6 py-3">
                                    <Badge
                                      ...
                                      className="text-xs"
                                    >
                                      {item.firstSource || item.source || item.eventName || "—"}
                                    </Badge>
                                  </td>
                                  <td className="px-6 py-3 text-sm text-neutral-500">
                                    {formatDate(item.createdAt || item.signedAt)}

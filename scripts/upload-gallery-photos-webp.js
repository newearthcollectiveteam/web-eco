...
async function uploadWebPPhotos() {
  try {
    ...
    if (!gallery) {
      throw new Error('Gallery not found! Please run upload-gallery-photos.js first.');
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (existingFile && existingFile.length > 0) {
          console.log(`⏩ [${i + 1}/${files.length}] ${filename} already in storage`);
          skipCount++;
        } else {
        ...
        if (result.length > 0) {
          console.log(`   📝 Updated database record for ${originalFilename} → ${filename}`);
          updateCount++;
        } else {
          console.log(`   ⚠️  No database record found for ${originalFilename}`);
        }

...
const resourceOptions = [
  ...
  {
    id: "photo_video",
    label: "Photography/video equipment",
    hasLocation: false,
  },
...
const discoveryOptions = [
  {
    id: "invitation",
    label: "Personal Invitation/Recommendation",
    detailPlaceholder: "From whom?",
  },
...
function CountdownTimer() {
  ...
  if (timeLeft.expired) {
    return <div className="text-sm text-white/60">This offer has expired</div>;
  }

  const TimeBlock = ({ value, label }: { value: number; label: string }) => (
    <div className="flex flex-col items-center">
      <div className="min-w-[3.5rem] rounded-lg border border-[#FACF39]/40 bg-black/60 px-3 py-2">
        <span
          className="text-2xl font-bold text-[#FACF39] md:text-3xl"
  ...
  return (
    <div className="flex items-center justify-center gap-2 md:gap-3">
      ...
      <span className="mb-5 text-xl font-bold text-[#FACF39]">:</span>
...
function WordCounter({ text, minWords }: { text: string; minWords: number }) {
  ...
  return (
    <div
      className={`mt-1 text-xs ${isValid ? "text-green-400" : "text-white/50"}`}
    >
      {wordCount} words ({minWords} minimum){" "}
      {isValid && <Check className="inline h-3 w-3" />}
    </div>
  );
}
...
function EmergenceFollowupForm() {
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1: // The Basics
        const hasBirthTime =
          formData.unknownBirthTime || formData.birthTime.trim().length > 0;
        return (
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <section className="relative overflow-hidden pt-24 pb-12">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 text-center">
          ...
          <p className="text-white/80">
            Thank you for attending The Emergence! Help us understand how we can
            best serve you and the collective.
          </p>
          <div className="mt-6 rounded-xl border border-[#FACF39]/40 bg-[#FACF39]/10 px-4 py-5 md:px-6">
            <div className="mb-4 flex items-center">
              <div className="flex flex-1 justify-center">
                <span className="text-2xl md:text-2xl">✨</span>
              </div>
              <p className="px-2 text-center text-sm font-medium text-[#FACF39]">
                <span className="md:hidden">
                  Submit the questionnaire to unlock
                  <br />
                  an opportunity for free lifetime access
                  <br />
                  to our online platform!
                </span>
                <span className="hidden md:inline">
                  Submit the questionnaire to unlock an opportunity to get free
                  lifetime access to our online platform!
                </span>
              </p>
              <div className="flex flex-1 justify-center">
                <span className="text-2xl md:text-2xl">✨</span>
            ...
            <p className="mb-3 text-xs tracking-wider text-white/70 uppercase">
              Offer expires in:
            </p>
            <CountdownTimer />
          </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-6">
        ...
        <div className="sm:hidden">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-sm text-[#FACF39]">
              {stepLabels[currentStep - 1]}
            </span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-[#FACF39]/20">
            <div
              className="h-full rounded-full bg-[#FACF39] transition-all duration-300"
            ...
            {stepLabels.map((_, idx) => {
              ...
              return (
                <div key={step} className="flex flex-shrink-0 items-center">
                  ...
                  {step < totalSteps && (
                    <div
                      className={`mx-2 h-1 w-10 rounded md:w-12 ${
      ...
      <div className="mx-auto max-w-3xl px-4 pb-24">
        <div className="rounded-lg border border-[#FACF39]/20 bg-black/60 p-6 backdrop-blur md:p-10">
          {/* Step 1: The Basics */}
          {currentStep === 1 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4 border-t border-white/10 pt-4">
                ...
                <label className="flex cursor-pointer items-center gap-3 text-white/80">
                  <Checkbox
                    className={checkboxClass}
                    checked={formData.isNomadic}
                    onCheckedChange={(checked) =>
                      updateField("isNomadic", !!checked)
                    }
                  />
                ...
                {formData.isNomadic && (
                  <div className="ml-8 space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateField("nomadicBaseLocation", e.target.value)
                      }
                    />
                  </div>
                )}
              ...
              <div className="space-y-4 border-t border-white/10 pt-4">
                ...
                <p className="text-sm text-white/50 italic">
                  For energetic blueprint calculations (Human Design, Astrology,
                  Gene Keys)
                </p>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="relative z-30 space-y-2">
                    ...
                    <DateTimePicker
                      ...
                      onChange={(date) =>
                        updateField("birthDate", date ? date.toISOString() : "")
                      }
                    />
                  </div>
                  <div className="relative z-20 space-y-2">
                    <label className="text-sm text-white/80">
                      Birth Time {!formData.unknownBirthTime && "*"}
                    </label>
                    <BirthTimePicker
                      name="birthTime"
                ...
                <div className="relative z-10 space-y-2">
                  ...
                  <LocationAutocomplete
                    ...
                    onSelect={(location) =>
                      updateField("birthLocation", location)
                    }
                  />
                </div>
              </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="mb-2 text-sm font-medium text-[#FACF39]/80">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {facilitatorRoles.map((role) => (
                        <label
                          key={role}
                          className="flex cursor-pointer items-center gap-3 text-white/80"
                        >
                          <Checkbox
                            className={checkboxClass}
                            checked={formData.identityRoles.includes(role)}
                            onCheckedChange={() =>
                              toggleArrayField("identityRoles", role)
                            }
                          />
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateField("identityRolesOther", e.target.value)
                  }
                />
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateField("newEarthMeaning", e.target.value)
                  }
                />
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateField("primaryIntention", e.target.value)
                  }
                />
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateField("sovereigntyMeaning", e.target.value)
                  }
                />
          ...
          {currentStep === 4 && (
            <div className="space-y-8">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white">
                  What unique gift or perspective do you want to share with this
                  community? *
                </label>
                <Textarea
                  className={inputClass}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateField("receiveFromCommunity", e.target.value)
                  }
                />
                <WordCounter
                  text={formData.receiveFromCommunity}
                  minWords={20}
                />
              </div>
              ...
              <div className="space-y-4 border-t border-white/10 pt-4">
                <div className="space-y-2">
                  <p className="text-sm text-white/70">
                    It is one of our highest intentions to build systems of
                    exchange that are reciprocal for all community members
                    involved.
                  </p>
                  <label className="text-sm font-medium text-white">
                    Under the right circumstances, are you open to share any
                ...
                <RadioGroup
                  ...
                  onValueChange={(value: string) =>
                    updateField("openToShareResources", value)
                  }
                  className="space-y-2"
                >
                  <div className="flex items-center space-x-3">
                    <RadioGroupItem
                      value="yes_free"
                      id="share-yes-free"
                      className="border-[#FACF39]/50 text-[#FACF39]"
                    />
                    <Label
                      htmlFor="share-yes-free"
                  ...
                  <div className="flex items-center space-x-3">
                    <RadioGroupItem
                      value="yes_depends"
                      id="share-yes-depends"
                      className="border-[#FACF39]/50 text-[#FACF39]"
                    />
                    <Label
                      htmlFor="share-yes-depends"
                      className="cursor-pointer text-white/80"
                    >
                      Yes, depending on the exchange/project
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3">
                    <RadioGroupItem
                    ...
                    <Label
                      ...
                      className="cursor-pointer text-white/80"
                    >
                      No, not ready to share yet
                    </Label>
                  </div>
                </RadioGroup>
              </div>

              {(formData.openToShareResources === "yes_free" ||
                formData.openToShareResources === "yes_depends") && (
                <>
                  <div className="space-y-4">
                    <label className="text-sm font-medium text-white">
                      ...
                      {resourceOptions.map((option) => (
                        <div key={option.id}>
                          <label className="flex cursor-pointer items-center gap-3 text-white/80">
                            <Checkbox
                              className={checkboxClass}
                              checked={formData.physicalResources.includes(
                                option.id
                              )}
                              onCheckedChange={() =>
                                toggleArrayField("physicalResources", option.id)
                              }
                            />
                            <span className="text-sm">{option.label}</span>
                          </label>
                          {option.hasLocation &&
                            formData.physicalResources.includes(option.id) && (
                              <Input
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateField("resourceOther", e.target.value)
                      }
                    />
                  </div>
                </>
            ...
            <div className="space-y-8">
                  ...
                  {discoveryOptions.map((option) => (
                    <div key={option.id}>
                      <div className="flex items-center space-x-3">
                        <RadioGroupItem
                          value={option.id}
                          id={`discovery-${option.id}`}
                          className="border-[#FACF39]/50 text-[#FACF39]"
                        />
                        <Label
                          htmlFor={`discovery-${option.id}`}
                          className="cursor-pointer text-white/80"
                        >
                          {option.label}
                        </Label>
                      ...
                      {formData.howFoundUs === option.id && (
                        <Input
                          ...
                          onChange={(e) =>
                            updateField("howFoundUsDetail", e.target.value)
                          }
                        />
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    {formData.howFoundUs === "other" && (
                      <Input
                        ...
                        onChange={(e) =>
                          updateField("howFoundUsOther", e.target.value)
                        }
                      />
                    )}
                  </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateField("existingConnections", e.target.value)
                  }
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="mb-2 text-xs text-white/50">
                  Helps us understand overlap and partnerships
                </p>
                <Textarea
                  className={inputClass}
                  rows={3}
                  ...
                  onChange={(e) =>
                    updateField("otherCommunities", e.target.value)
                  }
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateField("seekingConnections", e.target.value)
                  }
                />
              </div>
            </div>
          ...
          {currentStep === 6 && (
            <div className="space-y-8">
                  ...
                  {engagementOptions.map((option) => (
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          toggleArrayField("engagementStyles", option)
                        }
                      />
              ...
              <div className="space-y-2">
                ...
                <p className="mb-2 text-xs text-white/50">
                  Helps us build tools that serve you
                </p>
                <Textarea
                  className={inputClass}
                  rows={4}
                  ...
                  onChange={(e) =>
                    updateField("techAIRelationship", e.target.value)
                  }
                />
              ...
              <div className="space-y-2">
                ...
                <p className="mb-2 text-xs text-white/50">
                  Directly informs our ecosystem development
                </p>
                <Textarea
                  className={inputClass}
                  rows={4}
                  ...
                  onChange={(e) =>
                    updateField("improveSocialNetworks", e.target.value)
                  }
                />
                <WordCounter
                  text={formData.improveSocialNetworks}
                  minWords={20}
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <RadioGroup
                  ...
                  onValueChange={(value: string) =>
                    updateField("ecosystemContribution", value)
                  }
                  className="space-y-2"
                >
                  {ecosystemContributionOptions.map((option) => (
                    <div key={option.id}>
                      <div className="flex items-center space-x-3">
                        <RadioGroupItem
                          value={option.id}
                          id={`ecosystem-${option.id}`}
                          className="border-[#FACF39]/50 text-[#FACF39]"
                        />
                        <Label
                          htmlFor={`ecosystem-${option.id}`}
                          className="cursor-pointer text-white/80"
                        >
                          {option.label}
                        </Label>
              ...
              <div className="space-y-4 border-t border-white/10 pt-4">
                ...
                <p className="text-xs text-white/50">
                  How excited are you about what we're building?
                </p>
                <div className="px-2">
                  <Slider
                    value={[formData.excitementLevel]}
                    onValueChange={(value: number[]) =>
                      updateField("excitementLevel", value[0] ?? 5)
                    }
                    min={1}
                    max={10}
                    step={1}
                  ...
                  <div className="mt-2 flex justify-between text-xs text-white/50">
                    <span>1</span>
                    <span className="text-lg font-bold text-[#FACF39]">
                      {formData.excitementLevel}
                    </span>
                    <span>10</span>
                  </div>
                </div>
          ...
          {currentStep === 7 && (
            <div className="space-y-8">
              ...
              <div className="mb-4 space-y-2">
                <p className="text-sm text-white/70">
                  It is one of our highest intentions to protect your data as we
                  build this ecosystem. We will not share your data with any 3rd
                  parties unless explicitly expressed.
                </p>
              </div>

              <div className="space-y-4">
                <label className="text-sm font-medium text-white">
                  When we launch the ecosystem, how visible would you like your
                  profile to be? *
                </label>
                <RadioGroup
                  value={formData.profileVisibility}
                  onValueChange={(value: string) =>
                    updateField("profileVisibility", value)
                  }
                  className="space-y-2"
                >
                ...
                <div className="grid gap-2">
                  {communicationOptions.map((option) => (
                    ...
                    >
                      <Checkbox
                        className={checkboxClass}
                        checked={formData.communicationPrefs.includes(
                          option.id
                        )}
                        onCheckedChange={() =>
                          toggleArrayField("communicationPrefs", option.id)
                        }
                      />
                      <span className="text-sm">{option.label}</span>
                    </label>
              ...
              <div className="mt-8 rounded-lg border border-[#FACF39]/30 bg-[#FACF39]/10 p-4">
                <p className="text-sm text-white/80">
                  <span className="font-semibold text-[#FACF39]">
                    You're almost done!
                  </span>
                  <br />
                  Click "Submit" below to complete your questionnaire. We'll

...
const TABLE_GROUPS: TableGroup[] = [
  {
    label: "Auth & Team",
    description:
      "User profiles and authentication — team members, roles, approval",
    tables: [
      {
        name: "user_profile",
        columns: 14,
        purpose: "Team member profiles — linked to Supabase Auth, roles, phone",
        hasRLS: true,
      },
    ],
  },
  {
    label: "CRM & Contacts",
    description:
      "Master contact hub — all forms feed into central contacts table",
    tables: [
      {
        name: "contact",
        columns: 21,
        purpose:
          "Master CRM — email dedup, tags, consent tracking, unsubscribe tokens",
        hasRLS: true,
      },
      {
        name: "contact_source",
        columns: 8,
        purpose: "Multi-source tracking — which forms each contact submitted",
        fks: ["contact"],
        hasRLS: true,
      },
      {
        ...
        columns: 7,
        purpose: "Activity timeline — form submissions, notes, interactions",
        fks: ["contact"],
        hasRLS: true,
      },
      {
        name: "waitlist_intake",
        columns: 10,
        purpose: "Waitlist form submissions — name, email, message",
        fks: ["contact"],
        hasRLS: true,
      },
      {
        ...
        columns: 60,
        purpose: "Alignment questionnaire — 7 sections, 40+ fields",
        fks: ["contact"],
        hasRLS: true,
      },
    ],
    ...
    tables: [
      {
        name: "gallery",
        columns: 9,
        purpose: "Photo galleries — title, slug, cover image, display order",
        hasRLS: true,
      },
      {
        name: "gallery_image",
        columns: 11,
        purpose: "Individual photos — storage path, caption, dimensions",
        fks: ["gallery"],
        hasRLS: true,
      },
    ],
    ...
    tables: [
      {
        name: "event_waiver",
        columns: 14,
        purpose: "Signed liability waivers — signature data, consent tracking",
        hasRLS: true,
      },
    ],
  },
  {
    ...
    description:
      "Full-funnel user journey — anonymous visitors to known contacts",
    tables: [
      {
        name: "session",
        columns: 16,
        purpose: "Browser sessions — anonymous ID, UTM params, device info",
        fks: ["contact"],
        hasRLS: true,
      },
      {
        name: "event",
        columns: 16,
        purpose: "User interactions — page views, clicks, form submissions",
        fks: ["session", "contact"],
        hasRLS: true,
      },
      {
        ...
        columns: 6,
        purpose: "Identity resolution — links anonymous IDs to known contacts",
        fks: ["contact"],
        hasRLS: true,
      },
      {
...
const advisors: { security: Advisor[]; performance: Advisor[] } = {
  security: [
    {
      severity: "success",
      message: "All 13 tables have RLS enabled",
      url: null,
    },
    {
      severity: "warning",
      message:
        "RLS policies not yet defined on most tables (RLS enabled but no policies)",
      url: "https://supabase.com/docs/guides/database/database-linter?lint=0008_rls_enabled_no_policy",
    },
    {
      severity: "info",
      message: "Service role key used for admin operations (team management)",
      url: null,
    },
  ],
  performance: [
    {
      severity: "success",
      message: "Using connection pooler (Supavisor)",
      url: null,
    },
    {
      severity: "info",
      message: "Consider adding indexes on contacts.email, user_profile.email",
      url: "https://supabase.com/docs/guides/database/postgres/indexes",
    },
    {
      severity: "info",
      message: "Questionnaire table has 60 columns — monitor query performance",
      url: null,
    },
  ],
};
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-400">
          Supabase PostgreSQL — {ALL_TABLES.length} tables across{" "}
          {TABLE_GROUPS.length} domains
        </p>
      </div>
          ...
          >
            ...
            {expandedGroups.has(group.label) && (
              <table className="w-full text-sm">
                <thead>
                  <tr
                    className="border-b bg-white/5 text-left text-xs tracking-wider text-neutral-500 uppercase"
                        ...
                        {table.fks && table.fks.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {table.fks.map((fk) => (
                              <code
                                key={fk}
                                className="rounded bg-black/50 px-1.5 py-0.5 text-xs text-neutral-500"
                              >
                                → {fk}
                              </code>
                            ))}
            ...
            {advisors.security.map((advisor, idx) => (
              <div key={idx} className="flex items-start gap-3">
                ...
                {advisor.severity === "info" && (
                  <Zap className="mt-0.5 h-4 w-4 shrink-0 text-[#FACF39]" />
                )}
                <div className="flex-1">
                  ...
                  {advisor.url && (
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-xs text-[#FACF39] transition-colors hover:text-white"
                    >
      ...
      <div
        ...
        style={{
          ...
          backgroundColor: "rgba(250, 207, 57, 0.03)",
        }}
      >
        <p className="text-sm text-neutral-400">
          <strong className="text-neutral-300">Schema source:</strong>{" "}
          <code className="rounded bg-black/30 px-1.5 py-0.5 text-xs text-neutral-400">
            src/server/db/schema.ts
          </code>
          {" — "}All tables use Drizzle ORM with PostgreSQL. FK cascades handle
          child cleanup. All timestamps include timezone.
        </p>
        <a

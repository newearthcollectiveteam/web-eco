  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const sizeMB = (file.size / 1024 / 1024).toFixed(2);
        console.log(
          `🔄 [${i + 1}/${files.length}] Optimizing ${file.filename} (${sizeMB}MB)...`
        );
        ...
        const savings = (((file.size - info.size) / file.size) * 100).toFixed(
          1
        );

        console.log(
          `   ✅ Saved as ${basename(outputPath)} (${newSizeMB}MB, ${savings}% smaller)\n`
        );
    ...
    console.log(
      "   1. Review optimized .webp files in public/community-photos/"
    );
